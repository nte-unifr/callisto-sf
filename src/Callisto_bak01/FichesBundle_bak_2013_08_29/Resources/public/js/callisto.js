...
function emptyBasket() {
    var conf = confirm("Êtes-vous sur-e de vouloir effacer la sélection ?");
    if (conf) {
        $.jStorage.flush();
...
function removeItem(id) {
    var conf = confirm("Êtes-vous sur-e de vouloir supprimer cette fiche de la sélection ?");
    if (conf) {
...
function displayBasket() {
    ...
    selection += "</ul><br />"+'<div class="row"><div class="span4 offset2"><button type="button" class="btn btn-primary" onclick="javascript:emptyBasket()">Effacer la sélection</a></button></div>';
    if(index.length > 0) {
        content += selection;
    } else {
    ...
    // version imprimable
    content += '<div class="span6"><form method="post" action="'+ document.URL.split("/fiche/")[0] +'/print_panier"><textarea id="form_message" style="display: none;" name="form[message]">'+ content_mail +'</textarea><input type="submit" value="Version imprimable de la sélection" class="btn btn-primary"></form></div></div>';
    content += "</div>";
